...
ANNOTATION_FILES_DIR = "app/recommenders/similar_services/evaluation/manual_evaluation/storage/" \
                       "manual_evaluation_final_files"

GROUND_TRUTH_STORAGE_DIR = "app/recommenders/similar_services/evaluation/manual_evaluation/storage"
...
def update_ground_truth(ground_truth, annotation_file):
    ...
    for _, row in annotation_df.iterrows():
        ...
        if viewed_service_id in ground_truth:
            ground_truth[viewed_service_id]["similar_services"] = \
                list(set(similar_services_ids + ground_truth[viewed_service_id]["similar_services"]))
            ground_truth[viewed_service_id]["dissimilar_services"] = \
                list(set(dissimilar_services_ids + ground_truth[viewed_service_id]["dissimilar_services"]))
        else:
...
if __name__ == '__main__':
    ...
    annotation_files = glob.glob(f"{ANNOTATION_FILES_DIR}/**/*.xlsx")
    ...
    ground_truth_df.to_excel(f"{GROUND_TRUTH_STORAGE_DIR}/ground_truth.xlsx",
