...
from api.routes.update import update as structures_update
from api.settings import APP_SETTINGS, update_backend_settings
from api.recommender.similar_services.service_recommendation.recommendation_generation import create_recommendation

GOLD_SERVICES_PATH = \
    'api/recommender/similar_services/service_recommendation/evaluation/manual_evaluation/gold_services.csv'
OUTPUT_DIR = \
    'api/recommender/similar_services/service_recommendation/evaluation/manual_evaluation/storage/empty_manual_evaluations/'
GROUND_TRUTH_PATH = \
    'api/recommender/similar_services/service_recommendation/evaluation/manual_evaluation/storage/ground_truth.xlsx'
...
def get_recommendations(viewed_service_id):
    ...
    recommendations = create_recommendation(viewed_service_id, recommendations_num=6)
    ...
    return {
        ...
        'viewed_service_name': services_with_names[services_with_names["service_id"] == viewed_service_id].iloc[0]["name"],
        'recommendations': [
            [rec_item["service_id"], rec_item["name"]]
            for _, rec_item in services_with_names[services_with_names["service_id"] != viewed_service_id].iterrows()
        ]
    }
...
def get_ground_truth_relatability(viewed_service, recommended_service, relatability_ground_truth_df):
    try:
        similar_services = relatability_ground_truth_df[relatability_ground_truth_df["service_id"]==viewed_service]["similar_services"].values[0]
        dissimilar_services = relatability_ground_truth_df[relatability_ground_truth_df["service_id"]==viewed_service]["dissimilar_services"].values[0]
        relatability = None
...
def iterate_over_configs(config_dir):
    ...
    for path in tqdm(config_files):
        generate_excel(path, gold_service_ids, OUTPUT_DIR + path.split('/')[-1][:-5] + '.xls')
...
if __name__ == '__main__':
    ...
    iterate_over_configs('api/recommender/similar_services/service_recommendation/evaluation/manual_evaluation/configs/manual_evaluation_configs/')
