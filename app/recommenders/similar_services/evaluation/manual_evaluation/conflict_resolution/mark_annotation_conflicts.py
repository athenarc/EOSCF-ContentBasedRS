...
import pandas as pd
from app.recommenders.similar_services.evaluation.manual_evaluation.manual_annotations.create_manual_annotations import \
    ANNOTATORS_POOL

ANNOTATION_DIR = \
    'app/recommenders/similar_services/evaluation/manual_evaluation/conflict_resolution/storage/annotated_files/'

MARKED_CONFLICTS_DIR = \
    'app/recommenders/similar_services/evaluation/manual_evaluation/conflict_resolution/storage/marked_conflicts/'
...
def mark_annotation_conflicts(annotation_file):
    ...
    # Save file
    writer = pd.ExcelWriter(f"{MARKED_CONFLICTS_DIR}"
                            f"{annotation_file.split('/')[-1][:-5]}_marked_conflicts.xlsx", engine='xlsxwriter')
...
if __name__ == '__main__':
    ...
    annotation_files = glob.glob(ANNOTATION_DIR + '*')
