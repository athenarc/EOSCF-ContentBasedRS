...
CONFIG_DIR = \
    "app/recommenders/similar_services/evaluation/manual_evaluation/recommender_version_distances" \
    "/configs/best_text_attributes/"

RECOMMENDATIONS_STORE_DIR = \
    'app/recommenders/similar_services/evaluation/manual_evaluation/recommender_version_distances/' \
    'storage/produced_recommendations/'

HEATMAP_STORAGE_FILE = \
    'app/recommenders/similar_services/evaluation/manual_evaluation/recommender_version_distances/' \
    'storage/results/heatmap.png'
...
def plot_pairwise_similarities(dissimilarities_matrix, variations_names=None):
    mask = np.triu(np.ones_like(dissimilarities_matrix, dtype=bool))

    plt.figure(figsize=(int(0.75 * len(dissimilarities_matrix)) + 3,
                        int(0.38 * len(dissimilarities_matrix)) + 3))
    ...
    sns.heatmap(dissimilarities_matrix,
                ...
                vmin=0, vmax=1,
                annot=False if len(dissimilarities_matrix) > 6 else True,
                mask=mask)
...
def get_recommendations(viewed_service_id):
    ...
    recommendation = create_recommendation_set(viewed_service_id, recommendations_num=6)
