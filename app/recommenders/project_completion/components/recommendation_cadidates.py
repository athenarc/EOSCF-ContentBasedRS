from app.recommenders.project_completion.components.filtering import filtering
from app.recommenders.project_completion.initialization.association_rules import \
...
def get_recommendation_candidates(project_services):
    ...
    recommendations = [{"service_id": service_id, "score": 1}
            ...
            set().union(
                *rules[
                    rules.apply(lambda row: project_services == list(row["antecedents"]), axis=1)
                ]["consequents"].values)
        )]
