...
def filtering(db, resources, viewing_resource, purchased_resources):
    ...
    # Get non-published resources ids
    non_published_resources = db.get_non_published_services()

    # Remove every non-published resource existing in the resources and return them
    return resources.drop(labels=non_published_resources, errors='ignore')
